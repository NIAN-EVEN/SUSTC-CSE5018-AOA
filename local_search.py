...
def task(solution, func, firstMove, filename, kw, save=False):
    ...
    start = time.time()
    iter_pop, bestSoFar = iterativeLocalSearch(kw['evaluations'], kw['record_step'], solution, func, firstMove)
    if save:
...
def localSearch(solution, func, evaluation_num, evaluation_bound, record_step, firstMove):
    pop = [solution]
    ...
    while evaluation_num < evaluation_bound:
        ...
        if new_solution.score >= old_solution.score:
            num = int(evaluation_num/record_step) * record_step
            if pop[-1].evaluation_num == num:
                num += 1
            old_solution.evaluation_num = num
            pop.append(old_solution)
            break
        if evaluation_num / record_step >= len(pop):
            new_solution.evaluation_num = int(evaluation_num/record_step) * record_step
            pop.append(new_solution)
            # print("evaluation = %d\n" % evaluation_num, new_solution)
        old_solution = deepcopy(new_solution)
    return pop, evaluation_num

def iterativeLocalSearch(evaluation_bound, record_step, solution, func, firstMove):
    evaluation_num = 0
    solution.evaluation_num = evaluation_num
    iter_pop = [solution]
    bestSoFar = solution
    old_solution = deepcopy(solution)
    while evaluation_num < evaluation_bound:
        pop, evaluation_num = localSearch(old_solution, func, evaluation_num,
                                          evaluation_bound, record_step, firstMove)
        iter_pop.extend(pop[1:])
        if iter_pop[-1].score < bestSoFar.score:
            bestSoFar = iter_pop[-1]
        old_solution = Solution(doubleBridge(pop[-1].order))
